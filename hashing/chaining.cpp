    ...
    void display() {
        ...
        for (i = 0; i < _mod; i++) {
            if (!head[i]) {
                ...
                std::cout << "Key " << i << " has values = "<<endl;
                temp = head[i];
                while (temp->next) {
                    std::cout << temp->data << " ";
                    temp = temp->next;
...
int main() {
    ...
    std::cout << "Enter the size of Hash Table. = "<<endl;
    std::cin >> mod;
    ...
    while (loop) {
        ...
        switch (c) {
            case 1:
                std::cout << "Enter element to add = "<<endl;
                std::cin >> x;
                h = mychain.hash(x);
                h = std::abs(h);
                mychain.add(x, h);
                break;
            case 2:
                std::cout << "Enter element to search = "<<endl;
                std::cin >> x;
                h = mychain.hash(x);
                mychain.find(x, h);
                break;
            case 3:
                std::cout << "Enter element to generate hash = "<<endl;
                std::cin >> x;
                std::cout << "Hash of " << x << " is = " << mychain.hash(x)<<endl;
                break;
            case 4:
                mychain.display();
                break;
